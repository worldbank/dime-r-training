...
.pull-left[
<img src="img/raster.png"  width = 400>
]

.pull-right[
<img src="img/shapefile.png"  width = 300>
]
...
- To create the `worldmap` shapefile that you have in your `finalData` folder, we combined the data in `whr_panel` and the polygon in `worldmap`. Given what we have seen, this is as 
...
- As we saw earlier, shapefiles can contain poins, polygons or lines.
...
map<- ggmap(basemap) + 
  ...
  geom_sf(data = moz_projects,
          aes(color = sector), 
          inherit.aes = FALSE) +
  labs(title = "World Bank in Mozambique",
       color = "Project sector") +
  theme_void() + 
  scale_color_manual(values = wes_palette("GrandBudapest1", 4))
```
