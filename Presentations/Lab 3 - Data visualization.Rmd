...
\begin{enumerate}
  ...
  \item Load the \texttt{whr\_panel.csv} data set that were created in the last session.
...
\begin{block}{Exercise 2}
  \begin{enumerate}
    \item Create a histogram of variable \texttt{w\_area\_plots\_a} in data frame \texttt{whr\_simp}. Use the \texttt{hist()} function for that.
...
First, let's create a very simple plot with the happiness score on the Y axis and afreedom on the X axis.
...
### Exercise 5
 1. Use the ``ggplot()`` function with the ``anualHappy`` as the ``data`` argument, ``happy_score`` as ``y`` and ``year`` as ``x`` in ``aes()``
...
Ok, that worked, but was a really boring plot. Let's do a better one.

### Exercise 6: Part 1

Use the ``aggregate()`` function to average total house hold annual income, now, by two variables: `year` and `region`.
...
```{r, echo = T , eval= F}
# Separate datasets
sea <- anualHappy_reg[anualHappy_reg$region == "Southeastern Asia", ]
mena <- anualHappy_reg[anualHappy_reg$region == "Middle East and Northern Africa", ]
cee <- anualHappy_reg[anualHappy_reg$region == "Central and Eastern Europe", ]
...
```{r,  eval= F}
# Subset whr to 2017 and Rwamangana 35
...
### Exercise 7: Part 2
 ...
 1. Use the `region` varaible as color in ``aes()``.
...
\begin{figure}
\centering
  \includegraphics[scale = 0.7]{img/ggplotly1.png}
\end{figure}
...
### Bonus exercise - ggplotly
  1. Add a `text` argument to the`geom_point` `aes()` function and assign it to the `country` variable.

This argument won't be recognized by `ggplot()`, but will be by `ggplotly()`.
