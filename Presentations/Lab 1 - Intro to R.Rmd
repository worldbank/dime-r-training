...
whr <- read.csv(file.path(Data,"AirplanesVsBirds.csv"), 
                header = T)

whr <- read.csv(file.path(Data,"whr_panel.csv"), 
                header = T)
...
What about Python?
  ...
  * Python is even more flexible and has more users than R. So, why should I bother learning R?
...
\begin{enumerate}
  \item In RStudio, go to File > Import Dataset > From Text (Base) and \textcolor{red}{open the \texttt{lwh\_clean.csv}} file. Depending on your Rstudio version, it might be From CSV
  \item The file should be in Session Materials/R Track/DataWork/DataSets
  \item Change the name to `whr` on the import window
\end{enumerate}
...
\textcolor{red}{UPDATE SCREEN SHOTS}
...
\textcolor{red}{ADD SCREEN SHOOTS AND OUTPUT}
...
Create a new data set, called `whr2017`, that is a subset of the `whr` data set containing only data from the year 2015.
...
\textcolor{red}{add snapshot of enviroment with two values}
...
### Two an important concepts to take note:
 ...
 2. Print (display) is built into R. If you execute any action without storing it, R will simply print the results of that action but won't save anything in the memory.
...
This section will explore the most commun types of objects we use. Objects are the building blocks of R programming. You can create and manipulate them to explore your data and construct analytical outputs.

Objects can have different structures, that is, different ways to store data. Here are the ones we will cover today:
...
The `whr` and `whr2015` objects are both data frames. You can also construct a new data.frame from scratch by combining vectors.
...
\textcolor{red}{add screenshot}

In `whr`, we can see that `country` and `region` are factor variables. In your enviroment panel, there is the information of the type of all variables, and for factors the number of levels.
 ...
 * Boolean data is the result of a logical conditions and it assumes the values `TRUE` or `FALSE`.
 * Whenever you're using an `if` statment in Stata, you're implicitly using boolean data. The difference is that in R, this can be done in 2 steps.
 * Boolean data is not normally used directly in data frames.
