...
\begin{enumerate}
  ...
  \item The file should be in NISR R Training/Data/Final
...
* __Matrices:__ a combination of vectors of the same length and same type.
* __Data frames:__ a combination of different vectors of different types of the same length (the same as your data set in Stata)
...
# See the 67th element of the column
lwh$age_hhh[67]
...
Matrix indexing: typing `matrix[i,j]` will give you the element in the ith line and jth column
...
### You can also use it do select only the lines or columns
```{r}
...
### Names indexing
```{r}
# Or the 42th element of the hh_code column
lwh[42,"hh_code"]
```
...
Data frames also contain metadata stored in attributes. All objectes can have attributes, but data frame objects already come with a few. 
...
# there are also specialized functions to check and manipulate specific attibutes
names(df1)
```
