...
## Getting started
First, lets load the data that is going to be used in the training.
...
## Using packages
  ...
  * They can be shared through R's official repository - CRAN (13,000+ packages reviewed and tested) and many other online sources.
...
Use the `write.xlsx()` function to save the `happy_table` you table created in Exercise \textcolor{red}{XXX} into a xlsx file.
...
depvar_label <- "Happiness Score"
covar_labels <- c("GDP per capita",
                  "Freedom",
                  "Trust in government")
...
# Reg with year FE
reg2 <- lm(happy_score ~
           gdp_pc + freedom + trust_gov_corr + factor(year),
           data = whr)

# Reg with year and region FE
reg3 <- lm(happy_score ~
           gdp_pc + freedom + trust_gov_corr + factor(year) + factor(region),
           data = whr)
...
stargazer(reg1,
          ...
          font.size = "tiny",
          title = "Regression table",
          keep = c("gdp_pc", "freedom", "trust_gov_corr"),
          dep.var.labels = depvar_label,
          covariate.labels = covar_labels,
