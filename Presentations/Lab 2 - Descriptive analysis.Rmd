...
R has two types of "erros", `warnings` and actual `erros`:

  * `Errors` - break your code from running.
  * `Warnings` - are printed, but don't break your code.
  
But these are the standards, you can configure R to stop if you want.  
  
Warnings usally mean that nothing went wrong yet, but you should be careful. 
...
Use the `write.xlsx()` function to save the `happy_table` you table created in Exercise 4 into a xlsx file.
