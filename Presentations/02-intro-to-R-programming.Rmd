...
date: "The World Bank | [WB Github](https://github.com/worldbank) <br> November 2021"
output:
...
You'll never have to use  the equivalent of `clear all`
...
* __Note:__ Your project root is the directory that contains the `.Rproj` file (or `.git` directory if you are using git).
...
1. Go to the [OSF page of the course](https://osf.io/86g3b/) and download the file in: `R for Stata Users - November 2021` > `Data` > `DataWork.zip`
...
5. Now let's test if that worked. Load the `here` library and read the `csv` file `DataWork/DataSets/Final/whr_panel.csv` using the function `here()`.
]
...
* The meaning of the pipes is: *Pipes take the __output__ of the function at the left and pass it as the __input__ of the function at the right*
...
* For the purpose of this training, we will use `map()`. It is a function part of the tidyverse meta package

* We won't be covering loops in the main content of this session, but you can check the Annex of this presentation to learn how to write loops in R
...
**`map(X, function, ...)`:** applies `function` to each of the elements of `X`. If `X` is a data frame then `function` is applied column-wise while if it's a vector or a list it is applied item-wise. The output of `map()` is always a vector with the results.

  + **X:** a data frame, matrix or vector the function will be applied to
  + **function:** the function you want to apply to each of the elements of `X`
]
...
2. Use `map()` to get the maximum value of these columns

> Hints:
  + Remember the syntax of `map()`: `map(X, function)`. You can also try using pipes here
...
# List indexing revisited: `[]` vs `[[]]`

* The output of `map()` will be always a list

* Remember we said yesterday that you could use brackets to index list items? Well, we can also use double brackets

* While both will return the i-th element, a key difference is they return different data types:

  + `[]`: returns a list of one item containing the element selected
  + `[[]]`: returns the element selected itself

* Understanding this difference is crucial when working with `map()`
...
```{r, eval=FALSE}
my_list <- list(1, "hello")
my_list[2]   # this returns the string "hello" enclosed in another list
my_list[[2]] # this returns the actual string "hello"
```  
...
* The equivalent to that in R would be this
