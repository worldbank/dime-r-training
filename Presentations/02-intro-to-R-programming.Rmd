...
### Exercise 5: Loading the tidyverse

- Load the tidyverse packages in part 1 of your script
...
* For the purpose of this training, we will use `map()`. It is a function part of the tidyverse set of packages
...
* `map()` is part of the tidyverse setof packages. You need to either load the library `tidyverse` or `purrr` to enable it
...
* `map()` applies the function to each of the elements of `X`. If `X` is a data frame then the function is applied column-wise, while if it's a vector or a list it is applied item-wise

* The output of `map()` is always a vector with the results
...
* Recall the syntax of functions in R:
...
### Exercise 8: Custom functions and `map()` together

1. Use tidyverse's `select()` to select the columns `health_life_expectancy` and `freedom` in `whr`

2. Use `map()` combined with the `zscore` function to get the z-score of these two columns
