...
* __Notes:__
  ...
  + Your project root is the directory that contains the `.Rproj` file (or `.git` directory if you are using git).
...
x <- c(1.2, 2.5, 9.1, 5.8)
x %>% map(round) # Rounding the vector elements
...
> Hints:
  + Remember the syntax of `map()`: `map(X, function)`
  + Try using pipe concatenation here
...
* Remember we said yesterday that you could use brackets to index list items? we can also use double brackets

* While both will return the i-th element, a key difference is that they return different data types:

  + `[]`: returns a list of one item containing the element selected
  + `[[]]`: returns the element selected itself

* This difference is crucial when working with `map()`
...
xaringan_to_pdf("02-intro-to-R-programming.html")
```