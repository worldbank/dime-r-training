...
- The sessions include exercises. If time allows, you will be given 3-4 minutes to solve each exercise before we show the solution
...
- Every session has two TAs. For this session, our TA is __Maria (Mer) Reyes Retana__
...
1. Go to the OSF page of the course (https://osf.io/86g3b/) and download the file located in `R for Stata Users - 2024 March` > `Data` > `whr_panel.csv`

2. In RStudio, go to `File` > `Import Dataset` > `From Text (base)` and open the `whr_panel.csv` file. 
  + Depending on your version of RStudio, it might be `File` > `Import Dataset` > `From CSV`

3. Leave the name `whr_panel` to the data on the import window.
