...
date: "August 2018"
author: "Luiza Andrade & Leonardo Viotti"
output:
  beamer_presentation:
    theme: "Madrid"
    colortheme: "whale"
...
## Objectives
  ...
  \begin{itemize}
    ...
    \item To do them, go to the \texttt{Code} folder an open the file called \textit{Lab 2 - Coding for Reproducible Research.R}
  \end{itemize}

## Master Script

  \begin{itemize}
    \item A Master script is the easiest way to guarantee the replicability of your data work
    \item It has three main functions:
  \end{itemize}

  \begin{enumerate}
    \item Run all the scripts for your project
    \begin{itemize}
      \item Without a master script, you either need to have one extremely long script, or write instructions about the order in which they should be run
    \end{itemize}
\end{enumerate}
...
## Master Script
  ...
  \begin{enumerate}
    \setcounter{enumi}{1}
    \item Create a road map to all data work
    \begin{itemize}
      \item You should be able to reproduce all your work from raw data to all outputs by just running the master
      \item Anyone should be able to follow and understand your work by reading the master
    \end{itemize}
  \end{enumerate}
...
## Master Script
  ...
  \begin{enumerate}
  \setcounter{enumi}{2}
    \item Allow easy collaboration
    \begin{itemize}
      \item If we share a project over DropBox or OneDrive all team member have the same folder structure
      \item A master script allows multiple people to set their own file paths to the project folder
      \item This way anyone sharing the project folder can easily run your codes
    \end{itemize}
  \end{enumerate}
...
## Commenting
  ...
  \begin{itemize}
    \item Comments is text that R will ignore when running your code
    \item Comments are the difference between instructions that are easy to follow or impossible to understand
    \item Comments are used to document two things:
      \begin{enumerate}
        \item What is being done in a given section of the code
        \item Why it is being done
      \end{enumerate}
    \item Number 2 is what makes the difference between a well-commented code and a code that is just commented
  \end{itemize}
...
## Using packages
  ...
  \begin{block}{Exercise 6}
    ...
    \begin{itemize}
      \item TIP: use the helpfile to \texttt{install.packages} by typing \texttt{?install.packages} if you're not sure about how to do this.
    \end{itemize}
  \end{block}
...
knitr::include_graphics("img/whitespace.png", dpi = 220)
```
...
knitr::include_graphics("img/whitespace1.png", dpi = 170)
```
```{r fig.align="left", echo=F}
knitr::include_graphics("img/whitespace2.png", dpi = 190)
```
...
## Section switches
  ...
  * Adding switches is more efficient than commenting parts of the code out, because you can see all switches before running the code, so that avoids the mistake of saving a code with a commented section and forgetting to uncomment it lates
