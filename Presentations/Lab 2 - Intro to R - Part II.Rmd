...
## Why are we here today?
  ...
  * You could probably start doing some work in R by now, and you would figure out *what* should be in your code for it to work
  ...
  * This will give you a solid foundation to code in R
...
1. Go the `Lab 2 - Intro II.R` script. You can see that the first few lines in the script are the header, but they're not commented out.

2. Use the keyboard shortcut to comment the header of the script.

3. Use the keyboard shortcut to comment the header of the script again. What happened?
...
```
  ...
  * When do you use each of these things?
...
```{r}
  # Piping
  whr$year %>% log %>% summary
```


  * The output of one function can only be used as the input of another if they have the same format
  * To understand that, look at the first line of code in ``Lab 2 - Intro II.R``
