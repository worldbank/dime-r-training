...
.vlarge[
1. [Quick summary statistics](#sec3)
2. [Descriptive statistics tables](#sec4)
3. [Regression tables](#sec5)
]
...
Set folder paths to your computer
...
```{r, eval = F}
  # Load packages
  packages <- c("tidyverse", "skimr", "huxtable", "lfe")
  pacman::p_load(packages, character.only = TRUE)
```
...
```
```{r, echo=F, eval=T}
  table(census$region[1:23], census$state[1:23])
```
...
.large[
- The `skimr` package is very similar to `summary`.
- Its function is to present summary statistics for a dataset.
...
quick_xlsx(summary_stats_table,
           file = file.path(rawOutput, "summary-stats.xlsx"))

# Export to LaTeX
quick_latex(summary_stats_table,
           file = file.path(rawOutput, "summary-stats.tex"))
...
summary_stats_table <-
   summary_stats_table %>% 
   relocate(Variable) %>%  # Make veriable labels the first column
...
![https://cran.r-project.org/web/packages/huxtable/vignettes/themes.html](pics/themes.png){width=90%}
---
...
- `y ~ x1:x2` regresses variable `y` the interaction of covariates `x1` and `x2`
...
.exercise[
### Exercise `r fa("keyboard")`
Using the `census` data, run a regression of population, urban population and number of marriages. 
]
...
.exercise[
### Exercise `r fa("keyboard")`
Using the `census` data, run a regression of population, urban population and number of marriages controlling for region fixed effects. 
]
...
- Whenever a boolean is included in the list of covariates, it is treat as a dummy variable
...
# References and recommendations
  ...
  * Applied econometrics with R
  https://www.springer.com/us/book/9780387773162
