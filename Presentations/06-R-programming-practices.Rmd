...
* __Note:__ Your project root is the directory that contains the `.Rproj` fule
...
5- Save this new empty script in `DataWork` > `Code` as `exercises.R`

6- Now let's test if that worked. Load the `here` library read the `csv` file `DataWork/DataSets/Final/whr_panel.csv` using the function `here()`:
...
2. Add the following header before `read.csv(...)`: `Part 3: Loading data----`
...
* The output of `sapply()` is a vector with the results
