...
The below code shows two different ways of specifying the projection. The first way writes out the full projection. The second way uses a shorter way of writing the projection using an \textit{epsg code}. EPSG codes reference commonly used projections.
\scriptsize
```{r, eval=F}
# Way 1
...
## Basemap
We can add layers to basemap. The code is the same as ggplot, except here we start the code with \textit{ggmap()} instead of \textit{ggplot()}.
...
\scriptsize
Before making an interactive map, let's convert out dataframe of World Bank projects to a spatialdataframe. We don't necessarily need to do this, but it'll make our code simplier later on.
...
## Convert dataframe to spatialdataframe
We can directly plot spatialdataframes
```{r}
plot(wb_projects)
```
...
To make things simpler, subset the data to projects in Kenya. Leaflet works similarly to ggplot where you sequentially define different elements. However, instead of using + between elements, we use a pipe: %>%
...
kenya_projects <- wb_projects[wb_projects$recipients == "Kenya",]
...
Hints:
\begin{enumerate}
\item Use the "ad\_sector\_names" variable and the subset function to subset the dataset. For  example: subset(kenya\_projects, ad\_sector\_names == "Transport and storage")
...
## Cropping
Let's trim an object based on the spatial extent of another object. Let's say we only wanted trunk roads in Kenya We can use \texttt{crop()} from the \textit{sp} package.
```{r, eval=F, warning=FALSE, message=F, results='hide'}
trunk_roads_kenya <- crop(trunk_roads, extent(ken_adm1))
...
\scriptsize
```{r, warning=FALSE, message=F, results='hide', eval=F}
library(rgeos)
...
## Over Function
We have World Bank projects and the Africa polygon dataset. Let's say we want to add variables to the World Bank projects from the Africa polygon dataset. 

For this, we can use the \texttt{over()} function. We can use this to extract data from the polygons to each point.
\scriptsize
```{r, eval=F}
