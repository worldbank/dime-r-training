...
\begin{enumerate}
  ...
  \item Load the \texttt{lwh\_clean.csv} data set that were created in the last session.
    \begin{itemize}
      ...
      \item Remeber to disable the \texttt{INSTALL\_PACKAGES} switch in the Master if you already installed them. This will save you a lot of time.
    \end{itemize}
...
There are usually two distinct uses for plots:
  ...
  \begin{enumerate}
    \item \textbf{Exploratory analysis:} Quickly visualize your data in an insightful way
      ...
      \end{itemize}
    \item \textbf{Publication/Reporting:} Make pretty graphs for a presentation, a report, a dashboard. or to just show your boss something other than the laziest graph you could come up with
      \begin{itemize}
...
```
 ...
 2. Now, open yout ``Plots`` window. What does it show?
...
```{r, echo = F, warning = F}
ggplot(lwh, aes(y = w_area_plots_a,
                x = income_total_trim)) +
  geom_point()
```
