...
### Exercise 7
Call the package you just installed, as well as the other packages used in previous sessions.
...
## Looping
  ...
  * This is beyond today's session, and we will come back to it in our last day
...
## Why indent?
 * Even though R understands what unindented code says, it can be quite difficult for a human being
...
1. Add a line between the two arguments of the function (the vector of number and the round function)
...
```{r, eval = F}
  ...
  # Load data set
  lwh <- read.csv(file.path(finalData,"whr_panel.csv"),
                  header = T)
...
### Exercise 12
1. Turn off the switch that installs packages in your Master script

2. Add a line opening the data set in `PART 5` of your Master script
```{r, eval = F}
# Load data set
whr <- read.csv(file.path(finalData,"whr_panel.csv"),
                header = T)
...
3. Run the whole script. If it worked, your environment should include only the `whr` data set.
