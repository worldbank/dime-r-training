...
* Options for spherical distance calculations
- Formulas such as [haversine distance](https://en.wikipedia.org/wiki/Haversine_formula)
- [s2](http://s2geometry.io/) (Google library for working with spherical geometries)
]
...
* __[EPSG](https://epsg.io/)__ Assigns numeric code to CRSs to make it easier to reference. Here, WGS84 is `4326`. __sf package in R relies in ESPG__
]
...
* `geom_sf` is knows that we are in `WGS:4326` (spherical CRS), and adjusts map to minimize distortions (smart!). 
]
