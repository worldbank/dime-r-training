...
* __Great circle:__ Other formulas beyond s2, such as Haversine, Vincenty, etc. See the [geosphere](https://cran.r-project.org/web/packages/geosphere/geosphere.pdf) and [geodist](https://cran.r-project.org/web/packages/geodist/geodist.pdf) packages.
* __Projected:__ We can use a projected CRS, where units are in meters already.

```{r}
# s2
d1 <- st_distance(schools_sf[1,], schools_sf[2,])

# Nigeria-specific CRS
schools_utm_sf <- st_transform(schools_sf, 32632)
d2 <- st_distance(schools_utm_sf[1,], schools_utm_sf[2,])

# World mercator
schools_merc_sf <- st_transform(schools_sf, 3395)
d3 <- st_distance(schools_merc_sf[1,], schools_merc_sf[2,])
