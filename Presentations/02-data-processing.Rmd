...
if (Sys.getenv("USERNAME") %in% c("WB532468", "wb532468")){
  projectFolder  <- "/home/luis_eduardo/cs_projects/dime-r-training"
...
- To create a RStudio project click `File > New Project`, then:
...
- It could load your the last state of your environment. 
...
### To install and load a package you can run the following command:
...
# Let's load the two other packages:
...
The `here` package allows you to interact with your working directory. That's why it is important to set your RStudio project correctly. 
...
whr15 <- read_csv(here::here("DataWork", "DataSets", "Raw", "Un WHR", "WHR2015.csv")) %>% clean_names()
whr16 <- read_csv(here::here("DataWork", "DataSets", "Raw", "Un WHR", "WHR2016.csv")) %>% clean_names()
whr17 <- read_csv(here::here("DataWork", "DataSets", "Raw", "Un WHR", "WHR2017.csv")) %>% clean_names()
```
...
Filter or subsetting a dataset. We can extract our data when `region == "Western Europe"` using the `filter()` function. For example: 
...
A more elegant approach would be to use the `%in%` operator : 
...
One advantage of the filter command is that you can also integrate regular expressions. Let's say that we want to subset all regions' divisions that have `America` in their names. We can use the following: 
...
> Notice that I have used `head()` to show just the first 2 observations of the subset. If you want to save this subset you can assign it to an objet. For example `whr15_east <- + the code above`.
...
> The variable we created contains either `TRUE` or `FALSE`. <br> If we want to have it as a numeric (1 or 0), we could include `as.numeric()`
...
Let's imagine now that we want to create a variable at the region level (recall `bys gen` in Stata).In R, we can use `group_by()` before we mutate. So for this example, we going to pipe the following functions. 
...
```{r regions}
regions <- readRDS(here::here("DataWork", "DataSets", "Raw", "Un WHR", "regions.RDS"))
...
write.csv(whr_panel, 
          here::here("DataWork", "DataSets", "Final", "whr_panel.csv"), 
          row.names = FALSE) 
```
...
saveRDS(whr_panel, file = here::here("DataWork", "DataSets", "Final", "whr_panel.Rds"))
