...
\begin{enumerate}
  ...
  \item Open the script called \texttt{Lab 3} in \textcolor{red}{ DataWork > Code}. We've provided you with some code to build upon that will save you some time during this session.
\end{enumerate}
...
### Exercise 1
Load the ``lwh_panel.csv`` data set from \textcolor{red}{DataWork > DataSets > Raw}. Create an object called ``panel`` with this data set.
...
### Exercise 2
Use the function ``read.dta13()`` to load the ``endline_data_raw.dta`` data set from \textcolor{red}{DataWork > DataSets > Raw}. Create an object called ``endline`` with this data set.
...
### Exercise 17
Save the ``lwh`` data set to \textcolor{red}{DataWork > DataSets > Final}.
