...
* In this session, you'll be introduced to some basic concepts of data cleaning in R. The contents covered are:
    ...
    * \textcolor{red}{MERGING DATA}
    * \textcolor{red}{RESHAPING DATA}
    * Treating outliers
...
* The .Rdata format is also a nice alternative, which we will cover soon, but it isn't the standard
...
* TIP: all of these functions take a single argument, which is the object to be described.
...
* The ``panel`` data set has a lot more variables than we'll actually use for analysis
...
There are multiple ways to reshape data in R. We will use the `reshape()` function from the `stats` package.

It works very much like `reshape` in Stata. Here's the basic syntax:
...
\textcolor{red}{FINISH THIS EXERCISE !!!!}
