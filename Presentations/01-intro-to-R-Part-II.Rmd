---
title: "Introduction II - R programming"
...
* You can see all the objects in your memory t any point in the `Environment` pane
...
* Type `ls()` to print the names of all the object in memory

* To remove all objects, use `rm(list=ls())`
  + This is similar to using `clear all` in Stata
...
* For today's session:
  ...
  + On the `Files` panel, select `OSF Storage (United States)`
  + You'll notice that a button with the text `Download as zip` will appear next to `Filter`
