...
## Reshaping
  ...
  * The problem with the `gather` and `spread` commands is that they only reshape one column at a time
  * If you wanted to reshape multiple columns using these functions, you'd have to reshape them separate and merge back
  * The `reshape` function is a lot more similar to what we would do in Stata
...
```{stata, eval = F, size = "tiny"}
* Reshaping in stata
reshape wide  happy_score happy_rank, ///
              i(country) ///
              j(year)
```
...
```{stata, eval = F, size = "tiny"}
* Reshaping in stata
reshape long  happy_score happy_rank, ///
              i(country) ///
              j(year)
```
...
# Reshaping with reshape
happy_long <-
  reshape(select(happy_wide),
          direction = "long",
          idvar = "country",
          timevar = "year",
          varying = list(2:6),
          v.names = c("happy_score", "happy_rank"),
          sep = ".") 
...
# Save the data set
saveRDS(whr_panel,
        file = file.path(finalData, "whr_panel.Rda"))

# And load it again
whr_panel <- 
  readRDS(file.path(finalData, "whr_panel.Rda"))
