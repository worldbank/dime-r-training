...
> **Tip**: Use the `clean_names()` function in a pipe after you load a dataset as we did in the last slide.
...
.exercise[
**Exercise 3:** Filtering. Use `filter()` to extract only rows in one of these regions: (1) Eastern Asia and (2) North America. Hint: use the **or** operator (`|`): 
...
We use the `mutate()` function to add new variables. For example:
