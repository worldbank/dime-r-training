...
ADD IMAGE, DATA WORK PIPELINE AND IN OUTPUTS THERE'S A CSV AND A NICE EXCEL
...
ADD IMAGE OF CONSOLE INSTALLING
...
1. Go to https://www.osf.io/XXXXX and download the file

1. In RStudio, go to `File` > `Import Dataset` > `From Text (base)` and select the file `XXXXX.csv`

  + If you don't know where the file is, remember to check in your `Downloads` folder

1. Select `Import`

1. Go to https://www.osf.io/XXX, download the file, and repeat steps 2-3
  
ADD SCREENSHOT OF IMPORT BOX

---

# Introduction

You should have two dataframes loaded in the environment after this.
...
ADD SOMETHING ON GOOD CODE AND CODE CLARITY? AND ELEGANT?
...
ADD IMAGE, FROM UGLY CSV TO NICE EXCEL WITH LIBRARY BADGES
...
1. Use `datasummary_skim()` to create a descriptive statistics table for `XXXX`

```{r eval = FALSE}
datasummary_skim(XXXXX)
```
...
DROP EVAL=FALSE HERE SO RESULT IS DISPLAYED
...
```{r eval = FALSE}
datasummary_skim(XXXXX, type = "categorical")
```
...
Create a summary statistics table showing the nuber of observations, mean, standard deviation, minimum, and maximum for variables XXX, XXX, XXX, and XXX of the dataframe `XXX`

1. Use `datasummary()` for this:

```{r eval=FALSE}
datasummary(
  XX + XX + XX ~ N + Mean + SD + Min + Max,
  XXXXX
)
```
...
REMOVE EVAL= FALSE O SHOW RESULTS
...
```{r eval=FALSE}
datasummary(
  XX + XX + XX ~ N + Mean + SD + Min + Max, # this is the formula
  XXXXXX                                    # this is the data
)
```
...
Remember that both `datasummary_skim()` and `datasummary()` had an optional argument named *output*? We can  use it to specify a file path for an output file.
...
stats_table <- datasummary_skim(XXXXX, output = "huxtable")

# Export this new object to Excel with quick_xlsx()
quick_xlsx(stats_table, "quick_stats.xlsx")
```
...
The code belows shows how the table `stats_table` can be formatted:
...
SCREENSHOT OF OUTPUT IN DOCUMENTS FOLDER

ALSO SCREENSHOT OF TABLE OPENED IN EXCEL
...
We used `theme_basic()` to give a minimalist, basic theme to the table. Other available themes are:
...
- Other formatting options are:
  ...
  + Export in new Excel tabs instead of new files
  + Change variable and row names
  + Change the width of columns or height of rows
  + Color cells
  + Use text formatting like bold or italics
  
- This is explained in the libraries documentation:
...
ADD IMAGE OF PIPELINE ENDING IN EXCEL EXPORTED OUTPUT
...
ADD IMAGE OF PIPELINE ENDING IN VISUALIZATIONS
