...
We'll use one package in today's session: `dplyr` (really useful library for data wrangling).  
...
- Data wrangling is one of the most crucial and time-consuming aspects of Government analytics
...
1. Inyou new RStudio window, go to `File` > `Import Dataset` > `From Text(base)` and select again the file `department_staff_list.csv`
  ...
  + if you need to download it again, it's here: https://osf.io/g2ezw 

1. Make sure to select `Heading` > `Yes` in the next window

1. Select `Import`

1. Download this new file: https://osf.io/tvfyr and repeat steps 1-3  with it
...
- You can also load Excel files with the function `read.csv()` instead of using this point-and-click approach
...
- You can find the path of a file by right-clicking>properties>and seeing the location, or by 
